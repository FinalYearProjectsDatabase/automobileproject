        ...
        if(isset($_POST['service_vendor'])
            ...
            if(empty($_FILES["service_image"]["name"])){
                ...
                if(in_array($newExtension, $valid_Extension)){
                    if($fileSize <= 1000000){
                        $service_img = $service_id.time().".".$newExtension;
            ...
            // get the new service method and we pass the various parameters to create a new service
            $newService = $serviceObj->new_service($service_id, $service_description, $service_img, $service_vendor, $service_type, $service_location);

            // get the outcome of the method which is to return a json response and convert it in array
            $outcome = json_decode($newService);
